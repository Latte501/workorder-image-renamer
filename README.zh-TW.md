# convenience-store 工單圖檔自動改名

[English README](README.md)

把每天從 convenience-store 拍回來的紙本工單照片，用 Claude 視覺 API 自動辨識手寫欄位、
...
```
{mmdd} convenience-store{門市}({故障叫修} {修護內容詳細}){維修號碼}.jpg
```

範例：

```
0407 convenience-store同華(單杯)91430773.jpg
0403 convenience-store水龍吟(中WB不熱 32mm1 電磁閥1)91431308.jpg
```
...
```
{mmdd} convenience-store{門市}({故障叫修} {料件1}{數量1} {料件2}{數量2}...){維修號碼}.jpg
```
...
- `{mmdd}` 後接**半形空格**（不是 hyphen）；`convenience-store` 內部還是 hyphen。
- 修護內容空白時，括號內只放故障叫修。
- 多列修護內容用空格分隔。
